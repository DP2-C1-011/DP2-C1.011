...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("select avg(c.systemCurrencyBudget.amount) from Contract c where c.client.id = :clientId and c.draftMode = false")
	...
	@Query("select c from Contract c where c.client.id = :clientId and c.draftMode = false")
